...
public sealed class ConstrainedTriangulator : Triangulator
{
    ...
    internal Queue<EdgeConstraint> FindIntersectingEdges(EdgeConstraint constraint, int[] vertexTriangles)
    {
        ...
        // Aftershock patch: on degenerate cut faces this walk can ping-pong between triangles
        // forever, growing the queue until the process runs out of memory. A constraint can
        // cross each triangle at most once, so bail once we have walked past every triangle.
        int maxSteps = triangulation.GetLength(0) + 1;
        int steps = 0;
        while (!finalTriangleFound)
        {
            if (++steps > maxSteps)
            {
                Debug.LogWarning("Constraint walk did not terminate, exiting early.");
                break;
            }
            ...
            if (t < 0)
            {
                Debug.LogWarning("Constraint walk left the triangulation, exiting early.");
                break;
            }
